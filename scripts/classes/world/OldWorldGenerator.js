class OldWorldGenerator {
  ...
  static generate(world) {
    ...
    for (let i = 0; i < Application.CHUNK_GENERATION_PER_TICK; i++) {
      if (world.getState() != World.States.IDLE) {
        WorldGenerator.generateChunk(world, world.WorldGeneratorCache.slice[0], world.WorldGeneratorCache.slice[1]);
        WorldGenerator.bakeChunk(world, world.WorldGeneratorCache.slice[0], world.WorldGeneratorCache.slice[1]);
  ...
  static generateChunk(world, x, y) {
    if (!world.WorldGeneratorCache) {
    ...
    for (let localY = 0; localY < World.ChunkSize[1]; localY++) {
      let globalY = World.ChunkSize[1] * y + localY;
      if (!world.getBlocks()[globalY]) {
        world.getBlocks()[globalY] = [];
      }

      for (let localX = 0; localX < World.ChunkSize[0]; localX++) {
        let globalX = World.ChunkSize[0] * x + localX;
